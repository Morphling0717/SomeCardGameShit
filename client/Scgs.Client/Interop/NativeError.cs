...
internal static class NativeError
{
    ...
    internal static Exception CreateException(uint nativeCode) =>
        new ScgsNativeException(nativeCode, ReadLastError());
    ...
    {
        uint probe = ScgsV04NativeMethods.GetLastError(nint.Zero, 0, out ulong required);
        if (probe != (uint)NativeCode.BufferTooSmall ||
            required is 0 or > MaximumDiagnosticBytes)
        {
            return "The native library did not provide a diagnostic.";
        }
        ...
        try
        {
            ...
            {
                uint result = ScgsV04NativeMethods.GetLastError(
                    (nint)output,
                    (ulong)bytes.Length,
                ...
                {
                    return "The native library did not provide a readable diagnostic.";
                }
                ...
                {
                    return "The native library returned a non-UTF-8 diagnostic.";
                }
            }
        }
        finally
