...
- [ ] 己方 1～10 张手牌由相机相对 `BattlefieldHandRig` 排成前景弧线，保持最低屏幕卡高；悬停、选中和相邻让位不会把卡重新铺回桌面
- [ ] 对手手牌使用屏幕上方匿名牌背架，任何 actor 都不绑定 definition、instance ID 或私密材质
- [ ] 费用、攻击、生命和倒计时使用真实可见 `Label3D` 与独立徽章；自动证据同时验证节点、深度间距和最终 GPU ROI
- [ ] `BattlefieldViewportLayout` 固定四种验收尺寸的左右安全区；详情、日志和 HUD 显隐不再造成镜头缩放“呼吸”
- [ ] 战场占物理安全矩形至少 92% 宽、78% 高，格位、牌堆、主战者及双方领地不再表现为无归属的测试平板
- [ ] visual-suite 升级为 schema 4：原 11 状态加 `hand-one`、`hand-five`、`hand-ten`、`hand-hover`、`field-readability`，每张截图等待连续两个内容一致的 `FramePostDraw`
- [ ] 1280×720、1600×900、2560×1440、2560×1600 实拍经人工检查；缺字旧 golden 作废，新的 1600×900 golden 只能显式更新
- [ ] 四项 CI 在同一实现提交上全绿后交付 `SomeCardGameShit-gate4b-r2-windows-x86_64` 供第一次实机验收

上述条目只有在实现和同提交证据完成后才能勾选。R2 通过第一次实机验收后再规划 R3 的精细卡体、机械场地模型、材质灯光、响应链/动作演出、完整弹层和菜单统一。
