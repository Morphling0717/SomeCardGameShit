...
- [ ] 下一 Gate 实现全部规则能力、迁移旧规则测试到独立合成 fixture，并彻底删除 `midrange`／`advance` 定义、牌组键、旧卡图和菜单选项
- [ ] 新牌组可玩后以互换先后手 48～52%、赢家自身 T10～12 中位数等目标进行真实模拟和真人调优

Gate 5A 不冻结 C++ `CardId`、枚举或运行时 JSON 字段，也不改变 `IScgsGameSession`、C ABI、schema 1、14 个导出或 legacy v1 wire。legacy golden 后续必须改由协议 fixture 维持，不能借“兼容”为名继续暗藏旧产品牌组。
