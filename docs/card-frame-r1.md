...
状态：**已完成本机候选 GPU 检查，等待用户视觉批准**。实测范围及未覆盖项见
`docs/card-frame-r1-test-report.md`。本轮聚焦卡框的实体轮廓、
...
R1 的输入锁定公共结算期间，近端匿名牌背缩为半高，只表示手牌数量，避免挡住
后排待结算法术。私密手牌、远端牌背和默认产品均不改变尺寸；这不是保留私密
actor 的替代方案，原有清敏与主动揭示门继续生效。
