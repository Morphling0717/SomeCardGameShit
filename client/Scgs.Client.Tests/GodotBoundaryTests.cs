...
public sealed class GodotBoundaryTests
{
    ...
    public void WindowsAndMacEditorAndExportLayoutsResolveOnlyKnownLocations()
    {
        ...
        IReadOnlyList<string> windows = NativeLibraryLayout.CandidatePaths(
            ...
            projectNative);
        Assert.AreEqual(
            Path.Combine(root, "windows-export", "scgs_v04.dll"),
            windows[0]);
        CollectionAssert.Contains(
            windows.ToArray(),
            Path.Combine(projectNative, "windows-x86_64", "scgs_v04.dll"));
        ...
        CollectionAssert.Contains(
            mac.ToArray(),
            Path.Combine(projectNative, "macos-arm64", "libscgs_v04.dylib"));
