...
收尾的宝石高光／取证清理修正与最新交付包见末节；前面的历史数据不被回填。
...
`1750627` 四项主 CI 也已全绿，包含源码、导出和 ZIP 实启：
[run 33991969667](https://github.com/Morphling0717/SomeCardGameShit/actions/runs/33991969667)。
随后人工查看斜视录像约 1.42 秒的真实 GPU 帧，发现 Sapphire 大面积白色高光
会削弱白色数字对比。因此在该提交后仅调整 R1 专用晶体 shader：中心反射收敛、
边缘保留较强切面反光，改为非金属不透明晶体；没有修改模型、插画或文字坐标。
原片和失败帧保留作历史，不能说最初视频已通过此项检查。

同一次真实 MCP 重录发现正常结束会依次触发 Finish → Close → _ExitTree，
对 FramePostDraw 重复解除订阅，产生两条 nonexistent connection 错误。现改为
单一幂等释放；重建后连续完成三次录像，再显式取消一次，实际 runtime 日志
没有重复断开或其他错误。该修改仅用于独立设计取证窗口，不是战斗演出重做。

新录像仍为标明非对局的真实 GPU 设计展示，按实际时间戳 VFR 编码并核验：

| 卡 | 新捕获 ID | 真实／编码帧数 |
| --- | --- | --- |
| AP-11 | `20260905T211415979-141c870c` | 52／52 |
| LO-11 | `20260905T211452713-98b901c9` | 51／51 |
| NT-04 | `20260905T211512389-8a57f1d3` | 43／43 |

新文件在 `artifacts/card-frame-r1/evidence/gloss-final-design-turntables/`。
已查看 AP 对应斜角帧确认蓝色镶座不再成为白板；不把有限角度检查扩展成任意
光照下都合格。三张最新共享详情 GPU 近照为 `card-frame-r1-gloss-lo-close.png`、
`card-frame-r1-gloss-ap-close-actual.png`、`card-frame-r1-gloss-nt-close.png`。
没有 actual 后缀的 AP 首次抓取发生在设计展示清空详情之后，是空图，已排除。

最新回池报告 `20260905T211646703-9c287269` 同样实测 24 actor、6＋24 轮和
600 帧合成重场；failures 为空、同阶段资源引用稳定，全局资源数 144→144，
p95 4.2694 ms／max 4.3275 ms。它仍是上述 1440×1200 合成负载，不冒称真实
最大场或禁用 VSync 的压力测试。布局／晶体合同现 12 项，录像／生命周期合同
15 项；加 CI 路由合同，本机此次实际 39/39。Debug、Release 重建均零警告／错误。

最新 Windows 候选为
`artifacts/card-frame-r1/delivery/SomeCardGameShit-card-frame-r1-windows-x86_64-review.zip`，
SHA-256 `11fe4cd4566428ce66b3f84ce6ae780d4731bef0598037b63e0492921b5d4120`。
打包器对源导出、暂存及重新解包三次完成资产、14 导出与 MCP 隔离审计；包内
来源如实标为 `1750627 + 工作区`。新的两份真实启动记录分别位于
`artifacts/card-frame-r1/gloss-final-export-smoke/` 与 `gloss-final-zip-smoke/`。
两次真实硬件窗口 full-ui 均已通过，成功标记各恰好一次、14 类动作均有执行，
每份报告 157 命令／4 个终局，提前读取与私密泄露均为 0。包含本节的最终
提交 CI 必须独立确认，不沿用前两次 CI 结果；用户视觉批准仍未获得。
