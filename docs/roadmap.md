...
## Gate 3C：直接战场操作与中立结算投影 — 开发中

### 交互

- [ ] 高频操作采用点击/拖拽战场直操；复杂动作、部署组件与响应使用上下文选择
- [ ] 来源 → 动作 → 组件/格位/目标/预支只过滤引擎候选，点击与拖拽收敛到同一规范命令
- [ ] 最后一个必要选择后立即提交；无目标动作使用明确动作按钮，调度/投降保留专用确认
- [ ] Esc/右键逐步回退、悬停/固定详情、攻击连线、幽灵放置及非颜色单一的合法高亮
- [ ] 右侧行动列表退出主流程，改为可折叠详情与事件日志

### 隐私与验收

- [ ] `Resolving` 只显示中立公开战场并锁死输入，至少完整绘制两帧后提交
- [ ] 双方手牌身份、背面伏策身份、详情/日志/候选/回调均不能进入公共投影
- [ ] 初次揭示与真实操作者变化继续完全遮挡，新 viewer 主动揭示前零调用
- [ ] managed 覆盖每种动作的选择步骤/回退/规范命令；Godot 真实 signal E2E 覆盖自然终局、结果页重开与第二局投降终局
- [ ] Gate 3C schema v2 报告、Windows/macOS 导出与 ZIP 往返启动通过，制品改用 Gate 3C 名称
- [ ] 最终分支尖端四项 CI 全绿并将真实命令、数量、run 与 artifact 摘要写入 `TEST_REPORT.md`
