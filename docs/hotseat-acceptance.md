...
本清单把 Gate 3B/3C/4A 的历史基线、Gate 4B-R1 产品视觉契约、自动化结果与真人/物理设备验收分开记录。勾选只能依据同一提交的实现、测试日志或人工记录；准确运行数量与远端 CI 状态只以 [`../TEST_REPORT.md`](../TEST_REPORT.md) 为准。Gate 4B-R1 的实现自动化与四项 CI 已完成，包含本轮文档更新的最终分支尖端不得在本文预先宣称通过；尚无真人或物理设备证据的三项发布硬门继续保持未勾选。
