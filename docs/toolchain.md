...
| Python | 3.10 或更高 | legacy 兼容、制品/视觉素材审计、超时、Gate 3B/3C/4A full-match 及 Gate 4B-R2 visual/golden 契约测试 |
| Godot | **4.7.2 .NET** | Gate 4B-R2 authored 3D/2.5D 桌面热座、相机相对手牌、signal-driven full-match/visual smoke 与导出 |
...
Gate 4B-R2 继续使用 Compatibility renderer；默认启动 authored 3D/2.5D presenter，精确参数 `--legacy-2d-board` 只开放历史 2D 源码回归。前景手牌 rig、稳定镜头与 HUD 仍只使用 Godot 自身能力，不引入渲染插件、新 CMake option 或第二套客户端工具链。Windows/macOS 导出仍只包含默认 3D 产品入口。
