...
命令准备后进入不可交互的 `Resolving`，先绘制至少两个完整帧的中立公开战场，再延迟提交。此投影不引用 viewer 快照：双方手牌仅保留数量，所有背面伏策统一匿名，详情、日志、tooltip、metadata、候选和回调全部清除。只有初始揭示与实际操作者变化才进入完全不透明的 `Covered`；下一位主动揭示前不得读取其 view/query/events。
...
成功时只允许输出一次 `SCGS_GODOT_CI_SMOKE_OK` 并以 0 退出；缺库、错架构、ABI/schema 不符、C# exception 或 Godot error 必须失败。严格 Gate 3C 报告要求真实 signal E2E 覆盖全部 `ActionKind`：第一局自然终局后通过结果页 signal 重开，第二局再通过投降 signal 终局并释放两局 session；报告还验证点击/拖拽规范命令一致、最后选择后无通用确认、每次 `Resolving` 至少两个完整帧、零公共投影私密泄露及揭示前零 viewer 调用。CI 会在压缩前及 zip 解包后分别审计/启动导出包；真实结果以根目录 `TEST_REPORT.md` 为准。

本地视觉验收可额外传入 `--ci-screenshot=<绝对 PNG 路径>`；该参数只允许与 `--ci-smoke` 一起使用，并在恶意私密哨兵被清除后截取首个 `Resolving` 完整绘制帧。默认 headless smoke 仍等待两个 process-frame 栅栏，但不等待 `FramePostDraw`、也不生成截图。
...
Gate 3C 不包含正式卡图/音效/复杂动画、独立正式表现 JSON、主战技、普通主动能力、同时触发人工排序、触摸/手柄、Developer ID 签名/公证、Web 或 Linux 正式客户端。
